...
export function monthKey(date: Date | string | number) { const value = new Date(date); return `${value.getFullYear()}-${String(value.getMonth() + 1).padStart(2, "0")}`; }
...
export function weeklyPnl(trades: PerformanceTrade[], start: Date, end: Date) { return trades.filter(trade => { const date = new Date(trade.tradeDate); return date >= start && date <= end; }).reduce((total, trade) => total + toNumber(trade.pnl), 0); }
