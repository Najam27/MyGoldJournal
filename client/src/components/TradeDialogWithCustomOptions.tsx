...
function Field({ label, children, className = "", controlGroup = false }: { label: string; children: React.ReactNode; className?: string; controlGroup?: boolean }) {
  const labelId = `field-${label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  if (controlGroup) return <div className={`field ${className}`} role="group" aria-labelledby={labelId}><span id={labelId}>{label}</span>{children}</div>;
...
function CustomSelect({ category, value, onChange, options, placeholder, multi = false }: { category: string; value: string; onChange: (value: string) => void; options: string[]; placeholder?: string; multi?: boolean }) {
  ...
  // Slash is part of built-in values such as SBR/TJL1, so only pipe, comma,
  // and semicolon act as legacy multi-value separators.
  const selectedTags = (value || "").split(/\s*\|\s*|[,;]+/).map(tag => tag.trim()).filter(tag => tag && tag.toLowerCase() !== "none");
  const isSelected = (tag: string) => selectedTags.some(item => item.toLowerCase() === tag.toLowerCase());
  const toggleTag = (tag: string) => onChange(isSelected(tag) ? selectedTags.filter(item => item.toLowerCase() !== tag.toLowerCase()).join(" | ") : [...selectedTags, tag].join(" | "));
  const save = async () => {
    ...
    try {
      ...
      onChange(multi || category === "Mistake" ? (isSelected(next) ? selectedTags.join(" | ") : [...selectedTags, next].join(" | ")) : next);
      setDraft("");
  ...
  if (category === "Mistake" || multi) {
    return <div className="mistake-tags multi-select-tags"><div className="mistake-tag-grid">{choices.filter(item => item !== "None").map(item => <button type="button" key={item} className={isSelected(item) ? "selected" : ""} onClick={() => toggleTag(item)}>{item}</button>)}</div><div className="journal-value-add"><Input aria-label={`Add custom ${category} option`} value={draft} maxLength={70} onChange={event => setDraft(event.target.value)} placeholder={`+ Add custom ${category} option`} /><button type="button" title={`Save custom ${category} option`} aria-label={`Save custom ${category} option`} disabled={addOption.isPending || !draft.trim()} onClick={() => void save()}><Plus size={14} /></button></div></div>;
  }
  return <div className="journal-value-control"><select aria-label={category} value={value} onChange={event => onChange(event.target.value)}><option value="">{placeholder || `Select ${category.toLowerCase()}`}</option>{choices.map(item => <option key={item} value={item}>{item}</option>)}</select><div className="journal-value-add"><Input aria-label={`Add custom ${category}`} value={draft} onChange={event => setDraft(event.target.value)} placeholder={`+ Add ${category}`} /><button type="button" title={`Save custom ${category}`} aria-label={`Save custom ${category}`} disabled={addOption.isPending || !draft.trim()} onClick={() => void save()}><Plus size={14} /></button></div></div>;
}
