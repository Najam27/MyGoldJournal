...
vi.mock("@/lib/trpc", () => ({ trpc: { trades: { screenshot: { useQuery: () => ({ data: { url: "https://example.test/trade.png" }, isLoading: false }) }, removeScreenshot: { useMutation: () => ({ mutateAsync: vi.fn(), isPending: false }) } }, useUtils: () => ({ journal: { get: { invalidate: vi.fn() } }, trades: { list: { invalidate: vi.fn() } } }) } }));
...
describe("TradeLogWithViewer", () => {
  it("opens a card with trading detail and screenshot evidence without exposing internal metadata", () => {
    ...
    expect(screen.getByRole("button", { name: "Remove screenshot" })).toBeTruthy();
