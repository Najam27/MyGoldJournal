...
const mocks = vi.hoisted(() => ({ updateSettings: vi.fn(), markRead: vi.fn(), invalidate: vi.fn() }));

vi.mock("@/_core/hooks/useAuth", () => ({ useAuth: () => ({ isAuthenticated: true }) }));
vi.mock("@/lib/trpc", () => ({ trpc: { notifications: { get: { useQuery: () => ({ data: { settings: { goalAlerts: true, emailAlerts: false }, history: [{ id: 9, title: "Goal at risk", message: "Daily loss threshold is near.", createdAt: new Date("2026-08-12T00:00:00Z"), readAt: null }] } }) }, updateSettings: { useMutation: () => ({ mutateAsync: mocks.updateSettings, isPending: false }) }, markRead: { useMutation: () => ({ mutateAsync: mocks.markRead, isPending: false }) } }, useUtils: () => ({ notifications: { get: { invalidate: mocks.invalidate } } }) } }));
...
describe("NotificationCenter", () => {
  beforeEach(() => { mocks.updateSettings.mockReset(); mocks.markRead.mockReset(); mocks.invalidate.mockReset(); });
  afterEach(() => cleanup());
