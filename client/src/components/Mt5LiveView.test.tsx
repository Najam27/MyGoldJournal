  ...
  trpc: {
    mt5: {
      workspace: {
        useQuery: () => ({
          data: {
            connections: [{ id: 1, accountId: 12, accountName: "GFT 10K", label: "GFT Live", apiKey: "mt5_secret_connection_key_abcdefghijk", active: true, lastPing: new Date() }],
    ...
    useUtils: () => ({ mt5: { workspace: { invalidate: mocks.invalidate } } }),
  },
}));
...
describe("Mt5LiveView", () => {
  it("shows masked connection credentials, live positions, EA download, and safe journal handoff", () => {
    ...
    expect(ea.getAttribute("href")).toContain("GoldJournal_EA_3aafd714.mq5");
