...
describe("MT5 EA ingest", () => {
  ...
  it("treats naive MT5 broker timestamps as UTC+3 before PKT conversion", async () => {
    await expect(processMt5Payload(openPayload(key("utc-plus-five")))).resolves.toEqual({ status: 200, body: { ok: true, event: "open" } });
    expect(mocks.open).toHaveBeenCalledWith(77, 12, expect.objectContaining({ openTime: new Date("2026-07-11T06:30:00Z") }));
  });
