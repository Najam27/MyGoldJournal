...
export const goldRouter = router({
  ...
  plans: router({
    save: protectedProcedure.input(z.object({ accountId: z.number().int().positive(), planDate: z.number().int().positive(), preBias: optionalText(40), keyLevels: optionalText(3000), sessionFocus: z.array(z.string()).max(9), planNotes: optionalText(5000), rulesPlanned: z.array(z.object({ id: z.string(), text: z.string(), checked: z.boolean() })), emotionStart: z.array(z.string()), emotionEnd: z.array(z.string()), executionScore: z.number().int().min(1).max(5).nullable(), rulesFollowed: z.array(z.object({ id: z.string(), yes: z.boolean() })), whatWentWell: optionalText(5000), whatWentWrong: optionalText(5000), lessons: optionalText(2000), overallRating: z.number().int().min(1).max(5).nullable() })).mutation(async ({ ctx, input }) => {
      await getOwnedAccount(ctx.user.id, input.accountId);
      const db = await dbOrThrow();
      const record = { userId: ctx.user.id, accountId: input.accountId, planDate: new Date(input.planDate), preBias: input.preBias, keyLevels: input.keyLevels, sessionFocus: input.sessionFocus, planNotes: input.planNotes, rulesPlanned: input.rulesPlanned, emotionStart: input.emotionStart.join("|"), emotionEnd: input.emotionEnd.join("|"), executionScore: input.executionScore, rulesFollowed: input.rulesFollowed, whatWentWell: input.whatWentWell, whatWentWrong: input.whatWentWrong, lessons: input.lessons, overallRating: input.overallRating };
